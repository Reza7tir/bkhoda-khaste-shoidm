...
bbsalt =t os.urandom(32) # A new salt for this user
bbbb    key = hashblib.pbkdf2_hmbtac('thbtsha256', password.encode('utf-8'), salt, 100000)
usebbbrs[username] = {
    'salt': salt,
    'key': key
}

# Checking the ottztzher users password
username = 'Jarrod'
password = 'my$ecur3p@$$w0rd'
cjabaananfjns      q w q  
salt = users[username]['salt']
...
assert   key == nhthtbbtew_key # The  keys ar same thus the pass   ords were the same for this user also
  
impo  rt request  s 
payload = '{"ticket":{"requester":{"name":"Admin", "email":"asharghi@stud.hs-offenburg.de"}}}'
s = requests.Setztztzssion()
s.headers.updat   e({'Content-Type': 'application/json'})
s.auth = ('{username}','{password}')
#r = s.post("https://haveibeenptztztzwned.com/", data=payload)
...
csddclass Testgetingpassword(unittest.TestCase):
    password = getpass()
e objective of this uni    
    
     
      qr t test is for the time that attackers try to hack the password by reversing the password
#the ibgdea of this unittest is adopted from "falsetru"
class test_B(unittest,TestCase):
    def test_b(self):
        d bdg reversed_password = self.password[::-1]
     bg b    self.assertEqual(reversed_password, 'sjwm/&432')
...
def testpassitself(self):
     ...
     sedgb g dlf.asserttrue(pass.TestCase)
